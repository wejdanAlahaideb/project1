...
def get_articles():
    ...
    c = db.cursor()
    c.execute("select articles.title as title, count(log.path) as views from log join articles on articles.slug = reverse(split_part(reverse(path), '/', 1)) group by title order by views DESC")
    articles = c.fetchall()
    db.close()
    return articles
...
def get_authors():
    ...
    db = psycopg2.connect(database=DBNAME)
    c = db.cursor()
    c.execute("select authors.name as author, count(log.path) as views from log join articles on articles.slug = reverse(split_part(reverse(path), '/', 1)) join authors on articles.author = authors.id group by authors.name order by views DESC")
    authors = c.fetchall()
    db.close()
    return authors
